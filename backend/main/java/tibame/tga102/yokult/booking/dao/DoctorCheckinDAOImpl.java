...
public class DoctorCheckinDAOImpl {
	...
	public DoctorCheckinDAOImpl() {
		...
		// context.lookup("java:comp/env/mongodb/MyMongoClient");
	...
	public int insertOne(Patient patient) {
		Document doc1 = new Document("patientIdcard", patient.getPatientIdcard())
				.append("bookingNumber", patient.getBookingNumber()).append("doctorId", patient.getBookingNumber());
	...
	public Document selectOne(Doctor doctor) {
		...
		// mongoClient.getDatabase("patient").withCodecRegistry(pojoCodecRegistry);
		// MongoCollection<Patient> collection = database.getCollection("checkin",
		// Patient.class);
		// Patient patient = collection.find(eq("doctorId",
		// 1)).sort(ascending("bookingNumber")).first();
		// System.out.println(patient);

		// 得到document方式

		// Document doc = collection.find(eq("doctorId",
		// doctor.getDoctorId())).sort(ascending("bookingNumber")).first();
